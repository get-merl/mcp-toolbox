#!/usr/bin/env node
/**
 * Execution runner for context metrics tests.
 * This runner actually executes MCP tools and measures context usage
...
const DEFAULT_CONFIG = "scripts/context-metrics/config-exec.json";
...
async function run() {
      ...
      for (let i = 0; i < config.runsPerTask; i += 1) {
        ...
        try {
          ...
          log.steps.push({
            ...
            tool_definitions_count: tools.length,
            tool_definitions_bytes: toBytes(toolText),
  ...
  console.log(`Execution runs completed. Output: ${outputRoot}/${runStamp}`);
}
